...
const int ledposx = 26;
const int ledposy = 33;
const int lednegx = 32;
const int lednegy = 27;
...
void handle_Control(){
  if (server.arg("valueX")){
    ...
    Serial.print(",");
    Serial.println(y);
    // analogWrite(ledx, abs(x));
    // analogWrite(ledy, abs(y));
    write_Led(x, y);
  }
  server.send(200, "text/plain", "ok");
}

void write_Led(int x, int y){
  if (x==0){
    analogWrite(ledposx, 0);
    analogWrite(lednegx, 0);
  }else if(x>0) {
    ...
    analogWrite(ledposx, 0);
    Serial.println("Hola estoy en negx");
    analogWrite(lednegx, -x);
  }
  ...
  if (y==0){
    ...
    analogWrite(lednegy, 0);    
    Serial.println("Hola estoy en posy");
  }else{
    analogWrite(ledposy, 0);
    analogWrite(lednegy, -y);
  }
}
